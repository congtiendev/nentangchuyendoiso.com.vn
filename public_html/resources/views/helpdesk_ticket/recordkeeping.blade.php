...
@section('page-action')
    <div class="col-auto pe-0">
        <select class="form-select" id="projects" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" style="width: 150px;">
            ...
            <option value="{{route('helpdesk-tickets.search', 'Phê duyệt')}}" @if($status == 'Phê duyệt') selected @endif>Phê duyệt</option>
            <option value="{{route('helpdesk-tickets.search', 'Từ chối')}}" @if($status == 'Từ chối') selected @endif>Từ chối</option>
        </select>
    </div>
