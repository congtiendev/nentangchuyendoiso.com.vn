...
@section('page-action')
    <div>
        ...
        @permission('user import')
            <a href="#" class="btn btn-sm btn-primary" data-ajax-popup="true" data-title="{{ __('Nhập thông tin người dùng vào hệ thống') }}"
                data-url="{{ route('users.file.import') }}" data-toggle="tooltip" title="{{ __('Nhập thông tin người dùng vào hệ thống') }}"><i
                                        ...
                                        @permission('user login manage')
                                            @if ($user->is_enable_login == 1)
                                                <a href="{{ route('users.login', \Crypt::encrypt($user->id)) }}"
                                                    ...
                                                    <span class="text-danger"> {{ __('Cập nhật trạng thái người dùng') }}</span>
                                                </a>
            ...
            @permission('user create')
                <div class="col-md-3 All">
                    <a href="#" class="btn-addnew-project " style="padding: 90px 10px;" data-ajax-popup="true" data-size="md"
                        data-title="{{ __('Thêm mới '.($singular_name)) }}" data-url="{{ route('users.create') }}">
                        <div class="bg-primary proj-add-icon">
                            <i class="ti ti-plus my-2"></i>
                        </div>
                        <h6 class="mt-4 mb-2">{{ __('Thêm mới '.($singular_name)) }}</h6>
                        <p class="text-muted text-center">{{ __('Click để thêm mới '.($singular_name)) }}</p>
                    </a>
                </div>
            @endpermission
        @endauth
