{{ Form::open(['route' => ['pms.update', $pms[0]['id']], 'method' => 'PUT']) }}
<div class="modal-body">
    ...
    <div class="row">
        <div class="col-md-6 form-group">
            ...
            {{ Form::text('name', $pms[0]['name'], ['class' => 'form-control', 'placeholder' => __('Enter Name'), 'required' => 'required']) }}
        </div>
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="location" class="form-control select2">
                    ...
                    <option value="{{$key}}" @if($pms[0]['location_id'] == $key) selected @endif> {{ $value }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-md-12 form-group">
            ...
            {{ Form::textarea('description', $pms[0]['description'], ['class' => 'form-control', 'placeholder' => __('Enter Description'), 'required' => 'required', 'row' => 3]) }}
        </div>
        <div class="col-md-12">
            <div class="form-group">
                {{ Form::label('parts', __('Parts'), ['class' => 'col-form-label']) }}
                <div class="part_div">
                {{ Form::select('parts[]', $parts, $pms_role, ['class' => 'form-control multi-select', 'id' => 'part_id', 'multiple' => '', 'required' => 'required']) }}
                </div>
            </div>
        </div>
        <div class="col-md-12">
            {{ Form::label('tags', __('Tags'), ['class' => 'col-form-label']) }}
            {{ Form::text('tags', $pms[0]['tags'] , ['class' => 'form-control', 'id' => 'choices-text-remove-button']) }}
