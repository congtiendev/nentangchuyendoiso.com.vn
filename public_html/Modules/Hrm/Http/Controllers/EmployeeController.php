...
class EmployeeController extends Controller
{
    ...
    public function appoint()
    {
        if (!in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
            $employees = User::where('workspace_id', getActiveWorkSpace())
                ...
                ->select('users.*', 'users.id as ID', 'employees.*', 'users.name as name', 'users.email as email', 'users.id as id', 'branches.name as branches_name', 'departments.name as departments_name', 'designations.name as designations_name')
                ->get();
            return view('hrm::appoint.index', compact('employees'));
        } elseif (Auth::user()->isAbleTo('employee manage')) {
            $employees = User::where('workspace_id', getActiveWorkSpace())
                ->leftJoin('employees', 'users.id', '=', 'employees.user_id')
                ->leftJoin('branches', 'employees.branch_id', '=', 'branches.id')
  ...
  public function store(Request $request)
            ...
            try {
                $employee = Employee::create(
                    [
                        ...
                        'information_of_relatives' => $request['information_of_relatives'],
                        'internal_work_process' => $request['internal_work_process'],
                        'external_work_process' => $request['external_work_process'],
                        'certificate' => !empty($uploadCertificate['url']) ? $uploadCertificate['url'] : '',
                        'military_politics' => !empty($uploadMilitary['url']) ? $uploadMilitary['url'] : '',
                        'health' => !empty($uploadHealth['url']) ? $uploadHealth['url'] : '',
            ...
            try {
                $notification = CustomNotification::create(
                    [
                        ...
                        'content' => 'Chào ' . $user->name . ', bạn đã được tạo tài khoản thành công trên hệ thống ' . env('APP_NAME'),
    ...
    public function show($id)
    {
        if (Auth::user()->isAbleTo('employee show')) {
            try {
                ...
                return redirect()->back()->with('error', __('Không tìm thấy nhân viên'));
            }
    ...
    public function show2($id)
    {
        if (Auth::user()->isAbleTo('employee show')) {
            ...
            if (!empty($employee)) {
                ...
                return view('hrm::appoint.show', compact('employee', 'user', 'employeesId', 'documents', 'customFields'));
            } else {
                return redirect()->back()->with('error', __('Something went wrong please try again.'));
            }
        } else {
    ...
    public function createAppoint($id)
    {
        try {
            $id = Crypt::decrypt($id);
        } catch (\Throwable $th) {
        ...
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            return view('hrm::appoint.create', compact('employee', 'user', 'employeesId', 'branches', 'departments', 'designations', 'document_types', 'customFields'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function editAppoint($id)
    {
        ...
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            return view('hrm::appoint.edit', compact('employee', 'user', 'employeesId', 'branches', 'departments', 'designations', 'document_types', 'customFields'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            $this->saveLog($user, $employee, $changes, $action);
    ...
    private function saveLog($user, $employee, $changes,$action)
    {
        ...
        ActivityLogEmployee::create($logData);
    }
