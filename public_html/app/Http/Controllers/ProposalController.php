            ...
            if ($request->proposal_type == "product") {
                ...
                try {
                    $notification = CustomNotification::create([
                        'title' => 'Văn bản trình ký mới',
                        'content' => 'Bạn có một văn bản trình ký mới',
                        'link' => route('proposal.show', Crypt::encrypt($proposal->id)),
                        'from' => $proposal->created_by,
                        'send_to' => json_encode([$proposal->customer_id], JSON_NUMERIC_CHECK),
                        'type' => 'new_proposal',
                    ]);
                ...
                try {
                    $notification = CustomNotification::create([
                        ...
                        'link' => route('proposal.show', Crypt::encrypt($proposal->id)),
                        'from' => $proposal->created_by,
                        'send_to' => json_encode([$proposal->customer_id], JSON_THROW_ON_ERROR),
                        'type' => 'new_proposal',
                    ]);
                    UserNotifications::create([
                        'user_id' => $proposal->customer_id,
