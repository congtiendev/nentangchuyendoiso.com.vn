...
if (! function_exists('comapnySettingCacheForget')) {
    ...
    {
        try {
            $key = 'company_settings_'.getActiveWorkSpace().'_'.creatorId();
            Cache::forget($key);
        } catch (\Exception $e) {
...
if(! function_exists('AnnualLeaveCycle'))
{
    ...
    if (!function_exists('getNotification')) {
        function getNotification(): \Illuminate\Database\Eloquent\Collection
        {
            return CustomNotification::select('custom_notification.*', 'users.name as from_name', 'users.email as from_email', 'users.avatar as from_avatar')
    ...
    if (!function_exists('countNotification')) {
        function countNotification(): int
        {
            return UserNotifications::where('user_id', Auth::user()->id)->where('is_read', 0)->count();
        }
    }


    if (!function_exists('isReadNotification')) {
        function isReadNotification(string $id): ?UserNotifications
        {
            return  UserNotifications::where('user_id', Auth::user()->id)->where('notification_id', $id)->where('is_read', 0)->first();
        }
    }

    if (!function_exists('getDepartmentNameByDesignation')) {
        function getDepartmentNameByDesignation($designation_id = null)
        {
    ...
    if (!function_exists('getDesignationNameById')) {
        function getDesignationNameById($designation_id = null)
        {
            $designation = Designation::find($designation_id);
    ...
    if (!function_exists('addTakeRemindersNotification')) {
        function addTakeRemindersNotification()
        {
            ...
            foreach ($projects as $project) {
                $reminded = CustomNotification::where('type', 'remind_project_' . Auth::user()->id . "_" . $project->id)->first();
                if ($reminded) {
                    continue;
                }
                $daysleft = round((((strtotime($project->end_date) - strtotime(date('Y-m-d'))) / 24) / 60) / 60);
                if ($project->created_by != Auth::user()->id) {
                    $send_to =  [$project->created_by, Auth::user()->id];
                } else {
                    $send_to =  [Auth::user()->id];
                }
                if ($daysleft <= 3) {
                    $content = 'Dự án ' . $project->name . ' còn ' . $daysleft . ' ngày nữa sẽ hết hạn';
                    if ($daysleft <= 0) {
                        $content = 'Dự án ' . $project->name . ' đã hết hạn';
                    }
                    $notification = CustomNotification::create([
                        ...
                        'send_to' => json_encode($send_to, JSON_THROW_ON_ERROR),
                        'type' => 'remind_project_' . Auth::user()->id . "_" . $project->id,
                    ]);
    ...
    if (!function_exists('dateOfWeek')) {
        function dateOfWeek()
        {
            $dateOfWeek = [];
            $dateOfWeek['Thứ Hai'] = date('Y-m-d', strtotime('monday this week'));
            $dateOfWeek['Thứ Ba'] = date('Y-m-d', strtotime('tuesday this week'));
            $dateOfWeek['Thứ Tư'] = date('Y-m-d', strtotime('wednesday this week'));
            $dateOfWeek['Thứ Năm'] = date('Y-m-d', strtotime('thursday this week'));
            $dateOfWeek['Thứ sáu'] = date('Y-m-d', strtotime('friday this week'));
            $dateOfWeek['Thứ bảy'] = date('Y-m-d', strtotime('saturday this week'));
            $dateOfWeek['Chủ nhật'] = date('Y-m-d', strtotime('sunday this week'));
            return $dateOfWeek;
        }
    }
